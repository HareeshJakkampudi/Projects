  ...
  if(isset($_POST['submit']))
  {
      ...
      $array2=isset($_POST['dresscode_status']) ? $_POST['dresscode_status'] : '';
        ...
        if($null)
        {
              ...
              if($per1)
              {
                  $test0=mysqli_query($conn,"UPDATE `faculty_status` SET `Updated`='0',`NUpdations`='1',`status`='1' WHERE `f_name`='$name' and `date`='$date1' and `month`='$m' and `period`='$per1' and `section`='$sec1'");
              }
              else
              {
           ...
           if($array){
               foreach($_POST['character_status'] as $id=>$character_status)
             {
                ...
                $roll_number=$_POST['roll_number'][$id]; 
                  ...
                  $result=mysqli_query($conn,"insert into  cap_status(faculty,student_name,roll_number,attendance_status,date,month) values('$name','$student_name','$roll_number','$character_status','$date1','$m')");
                if($result)
                {
     ...
     try{
          ...
          $roll_number=$_POST['roll_number'][$id];       
           ...
           $result=mysqli_query($conn,"insert into cap_status(faculty,student_name,roll_number,attendance_status,date,month) values('$name','$student_name','$roll_number','$attitude_status','$date1','$m')");
          if($result)
          {
            $flag=1;
                         $month=calculate($m);
   ...
   try{
      ...
      foreach($_POST['dresscode_status'] as $id=>$dresscode_status)
      {
        ...
        $result=mysqli_query($conn,"insert into cap_status(faculty,student_name,roll_number,attendance_status,date,month) values('$name','$student_name','$roll_number','$dresscode_status','$date1','$m')");
        if($result)
        {
          $flag=1;
                     $month=calculate($m);
...
<link rel="stylesheet" href=" https://cdn.jsdelivr.net/npm/pretty-checkbox@3.0/dist/pretty-checkbox.min.css">
    ...
    <body>
      ...
      <?php
            ...
            if($flag) { 
              echo "<script type='text/javascript'>alert(\"Thank You Updated Successfully!!\")</script>";
      ...
      echo '</script>';
                   ...
                   echo "<script type='text/javascript'>alert(\"Thank You No Updations were made!!\")</script>";
                echo '<script type="text/javascript">';
      echo 'window.location.href="dashboard.php";';
      echo '</script>';
